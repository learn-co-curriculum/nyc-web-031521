    ...
    newListForm.addEventListener("submit", function(evt){
        ...
        fetch("http://localhost:3000/lists", {
            ...
            .then(function(newList){
                ...
                // REACT CODE -> setState(copyOfLists)
...
function handleDeleteFetch(list, user){
    ...
    fetch(`http://localhost:3000/lists/${list.id}`, {
        ...
        .then(function(emptyObj){
            // UPDATE THE USER OBJECT IN MEMORY
            let filteredLists = user.lists.filter((listObj) => {
                return listObj.id !== list.id
            })

            user.lists = filteredLists
            
            // DELETE THE LIST OFF OF THE DOM
            setLists(user)
        })
}
...
// let addedItems = [...list.items, "NEW STUFF"]

// let deleteItems = list.items.filter(item => {
    // return item !== theItemToDelete
// })

// fetch(PATCH "/lists/:ID", {
    // BODY: JSON.stringify({items: addedItems/deleteItems})
// })
// 